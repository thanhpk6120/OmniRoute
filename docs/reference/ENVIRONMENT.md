...
| `OUTBOUND_SSRF_GUARD_ENABLED`           | `false`                 | `src/shared/network/outboundUrlGuard.ts` | Legacy alias for the outbound SSRF guard. **Default disabled** so self-hosted providers work out of the box. Set to `true` to re-enable the guard for shared/public deployments.                                                                                                                                                                                   |
| `OMNIROUTE_ALLOW_PRIVATE_PROVIDER_URLS` | `true`                  | `src/shared/network/outboundUrlGuard.ts` | Allow provider URLs pointing to private/local networks (localhost, 192.168.x.x, 10.x.x.x, etc.). **Default enabled** so self-hosted providers (LM Studio, Ollama, vLLM, Llamafile, Triton, SearXNG, internal LAN routers, SSH-tunnelled localhost) work without configuration. Set explicitly to `false` to enforce the strict SSRF guard.                         |
